...
/**
 * Hook for fetching PUBLIC page sections (client-side)
 * Uses the public API endpoint - no auth required
 */
export function usePublicPageSections(pageIdentifier: 'homepage' = 'homepage') {
  const { data: sections, isLoading, isError, isFetching } = useGetActivePageSectionsQuery(pageIdentifier);
